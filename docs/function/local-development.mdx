...
If you build HTTP applications with [API Gateway HTTP events](handlers.mdx#api-gateway-http-events), `serverless bref:local` is a bit unpractical because you need to manually craft HTTP events in JSON.
