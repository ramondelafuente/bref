export const description = 'Learn how to deploy serverless Laravel applications on AWS Lambda using Bref.';
...
First, make sure you have followed the [Installation guide](../installation.mdx) to create an AWS account and install the necessary tools.
...
Follow [the deployment guide](/docs/deploy.mdx#deploying-for-production) for more details.
...
Then follow [this section of the documentation](/docs/environment/storage.mdx#deploying-dynamodb-tables) to deploy your DynamoDB table using the Serverless Framework.
