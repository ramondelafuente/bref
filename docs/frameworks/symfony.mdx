export const description = 'Learn how to deploy serverless Symfony applications on AWS Lambda using Bref.';
...
First, **follow the [Installation guide](../installation.mdx)** to create an AWS account and install the necessary tools.
...
Since [the filesystem is readonly](/docs/environment/storage.mdx) except for `/tmp` we need to customize where the cache and logs are stored in
...
The application is now ready to be deployed. Follow [the deployment guide](/docs/deploy.mdx).
...
By default, Symfony logs in `stderr`. That is great because Bref [automatically forwards `stderr` to AWS CloudWatch](/docs/environment/logs.mdx).
...
To learn more about all this, read the [environment variables documentation](/docs/environment/variables.mdx).
...
For more details, see the [Websites section](/docs/websites.mdx) of this documentation
...
Now, you can follow [this section of the documentation](/docs/environment/storage.mdx#deploying-dynamodb-tables) to deploy your DynamoDB table using the Serverless Framework.
